...
class Db
{
     ...
     * 关联join查询
     *
     * @param $a    例如：hui_admin_log l ，必须要包含表前缀
     * @param $b    条件
     * @param $type 类型
     *
     * @return $this
    ...
    {
        $field       = isset($this->key['field']) ? str_replace('hui_', $this->config['db_prefix'],
            $this->key['field']) : ' * ';
    ...
    public function max($field = '')
    {
        ...
        $where     = ! empty($this->key['where']) ? ' WHERE '.self::getWhereArr() : '';
        $limit     = ' LIMIT 1';
        $sql       = 'SELECT MAX('.$field.') AS hui_max  FROM '.$this->get_tablename().$join.$where.$limit;
        $findquery = $this->execute($sql);
        if ( ! empty($findquery)) {
            $res = $findquery->fetch(PDO::FETCH_ASSOC);
            $max = ! empty($res['hui_max']) ? $res['hui_max'] : 0;
    ...
    public function min($field = '')
    {
        ...
        $where     = ! empty($this->key['where']) ? ' WHERE '.self::getWhereArr() : '';
        $limit     = ' LIMIT 1';
        $sql       = 'SELECT MIN('.$field.') AS hui_min  FROM '.$this->get_tablename().$join.$where.$limit;
        $findquery = $this->execute($sql);
        if ( ! empty($findquery)) {
            $res = $findquery->fetch(PDO::FETCH_ASSOC);
            $min = ! empty($res['hui_min']) ? $res['hui_min'] : 0;
    ...
    public function avg($field = '')
    {
        ...
        $where     = ! empty($this->key['where']) ? ' WHERE '.self::getWhereArr() : '';
        $limit     = ' LIMIT 1';
        $sql       = 'SELECT AVG('.$field.') AS hui_avg FROM '.$this->get_tablename().$join.$where.$limit;
        $findquery = $this->execute($sql);
        if ( ! empty($findquery)) {
            $res = $findquery->fetch(PDO::FETCH_ASSOC);
            $avg = ! empty($res['hui_avg']) ? $res['hui_avg'] : 0;
    ...
    public function sum($field = '')
    {
        ...
        $where     = ! empty($this->key['where']) ? ' WHERE '.self::getWhereArr() : '';
        $limit     = ' LIMIT 1';
        $sql       = 'SELECT SUM('.$field.') AS hui_sum FROM '.$this->get_tablename().$join.$where.$limit;
        $findquery = $this->execute($sql);
        if ( ! empty($findquery)) {
            $res = $findquery->fetch(PDO::FETCH_ASSOC);
            $sum = ! empty($res['hui_sum']) ? $res['hui_sum'] : 0;
